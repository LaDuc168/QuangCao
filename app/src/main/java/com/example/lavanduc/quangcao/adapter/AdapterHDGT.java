...
public class AdapterHDGT extends BaseAdapter {
    Context myContext;
    ArrayList<HoatDongGT> mang;

    public AdapterHDGT(Context myContext, ArrayList<HoatDongGT> mang) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        HoatDongGT khachSan=mang.get(position);

        Bitmap bitmap= BitmapFactory.decodeByteArray(khachSan.getAnh(),0,khachSan.getAnh().length);
        img.setImageBitmap(bitmap);
        txttenks.setText(khachSan.getTen());
        txtdanhgia.setText(khachSan.getLuotthich()+" lượt thích");
        return convertView;
    }
}
